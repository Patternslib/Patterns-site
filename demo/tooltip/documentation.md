...
| `source` | `title` | `auto` `ajax` `content` `title` | Select where the contents of the tooltip is taken from. AJAX loading of the link target, the contents of element or its title attribute. The `auto` option will revert to `ajax` or `content`, depending on whether the link target points to an external URL or to a fragment inside the current page. | Mutually exclusive |
